...
Both operations handle the two-step Proget flow (multipart file upload + application registration) in a single node. Feed them binary data from any upstream node (HTTP Request, Read Binary File, S3, ...). The APK is validated before upload and its filename is sanitized.
...
- UUIDs and IMEIs are validated before being interpolated into API paths.
